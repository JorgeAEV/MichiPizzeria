...
class CategoriasController {
    mostrar_todos_categorias(req, res) {
    ...
    actualizarCategoria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            console.log(id);
            const resp = yield database_1.default.query("UPDATE categorias set ? WHERE id = ?", [req.body, id]);
            res.json(resp);
        });
    }
    eliminarCategoria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const { id } = req.params;
            const resp = yield database_1.default.query(`DELETE FROM categorias WHERE id = ${id}`);
            res.json(resp);
        });
    }
}
